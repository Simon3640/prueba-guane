...
from app.schemas import (ExpenseCategoryCreate,
                         ExpenseCategoryInDB,
...
async def get_categories(
    *,
    ...
    try:
        categories = await expense_category_service.get_multi(current_user, skip=skip, limit=limit)
    except BaseErrors as e:
        raise HTTPException(e.code, e.detail)
    return categories
...
async def update_category(
    ...
    *,
    ...
    try:
        category = await expense_category_service.update(current_user, obj_in=category, id=id)
    except BaseErrors as e:
