...
class CRUDUser(CRUDBase[User, UserCreate, UserUpdate, UserRules]):
    ...
    async def create(self, *, obj_in: UserCreate) -> User:
        async with in_transaction() as db:
            ...
            ) or await self.get_by_email(email=obj_in.email)
    ...
    async def update(self, who: User, *, obj_in: UserUpdate | dict, id: int) -> User:
        async with in_transaction() as db:
            db_obj = await self.get(who, id=id)
            user: User = await self.get_by_username(
                obj_in.username
            ) or await self.get_by_email(obj_in.email)
            # Error
    ...
    async def update_password(
        ...
        *,
        ...
        async with in_transaction() as db:
            ...
            update_data = {"hashed_password": bcrypt.get_password_hash(password)}
    ...
    async def authenticate(self, *, username: str, password: str) -> User:
        async with in_transaction() as db:
            user: User = await self.get_by_email(
                username
            ) or await self.get_by_username(username)
